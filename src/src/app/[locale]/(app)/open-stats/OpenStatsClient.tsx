...
export default function OpenStatsClient() {
  ...
  const userPercentile = useMemo(() => {
    ...
    if (stats.isTime) percentile = 100 - percentile;
    return Math.max(1, Math.min(100, percentile));
  }, [stats, userNumericScore]);

  if (!isMounted || isUserLoading) {
    return <div className="flex h-screen w-full items-center justify-center"><LoaderCircle className="h-12 w-12 animate-spin text-primary" /></div>;
  }
    ...
    <div className="flex flex-col h-full bg-background">
      <header className="flex flex-col border-b bg-card/50 backdrop-blur-md sticky top-0 z-30">
        <div className="flex items-center justify-between p-4 md:p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-2xl font-bold font-headline tracking-tight md:text-3xl text-primary">{t('title')}</h1>
              <p className="text-xs text-muted-foreground hidden md:block">{workout === "0" ? t('description') : `${year}.${workout}`}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Select value={workout} onValueChange={setWorkout} disabled={isLoading}>
              <SelectTrigger className="w-[120px] h-9"><Activity className="h-3 w-3 mr-2" /><SelectValue /></SelectTrigger>
              <SelectContent>
        ...
        <div className="flex items-center gap-4 px-4 pb-3 md:px-6 overflow-x-auto no-scrollbar border-t pt-3 bg-muted/20">
          <div className="flex items-center gap-2 shrink-0"><Filter className="h-3 w-3 text-muted-foreground" /><span className="text-[10px] uppercase font-bold text-muted-foreground mr-2">{t('filters.label')}</span></div>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8">
        {error && (
          <div className="bg-destructive/10 border border-destructive/20 p-4 rounded-lg flex items-center gap-3 text-destructive">
            ...
            <p>{error}</p>
            <Button variant="outline" size="sm" onClick={() => fetchLeaderboard(stage, parseInt(year), parseInt(workout), parseInt(division), parseInt(region), parseInt(scaled))} className="ml-auto">{t('leaderboard.retry')}</Button>
          </div>
        )}

        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4">
          <Card className="bg-primary/5 border-primary/20"><CardHeader className="pb-2"><CardDescription className="flex items-center gap-2 text-xs uppercase tracking-wider font-bold"><Users className="h-3 w-3" /> {t('metrics.athletes')}</CardDescription><CardTitle className="text-3xl font-bold">{isLoading ? "..." : (stats?.totalCount || 0)}</CardTitle></CardHeader></Card>
          <Card><CardHeader className="pb-2"><CardDescription className="text-xs uppercase tracking-wider font-bold">{t('metrics.median')}</CardDescription><CardTitle className="text-3xl font-bold text-blue-400">{isLoading ? "..." : formatScore(stats?.median || 0, stats?.isTime || false)}</CardTitle></CardHeader></Card>
          <Card><CardHeader className="pb-2"><CardDescription className="flex items-center gap-2 text-xs uppercase tracking-wider font-bold"><Trophy className="h-3 w-3 text-yellow-500" /> {t('metrics.top10')}</CardDescription><CardTitle className="text-3xl font-bold text-yellow-500">{isLoading ? "..." : formatScore(stats?.p90 || 0, stats?.isTime || false)}</CardTitle></CardHeader></Card>
          <Card className="bg-yellow-500/10 border-yellow-500/30"><CardHeader className="pb-2"><CardDescription className="font-bold text-yellow-600 text-xs uppercase tracking-wider">{t('metrics.elite')}</CardDescription><CardTitle className="text-3xl font-bold text-yellow-600">{isLoading ? "..." : formatScore(stats?.p99 || 0, stats?.isTime || false)}</CardTitle></CardHeader></Card>
          ...
          <Card className="md:col-span-4 lg:col-span-1 border-accent/30 shadow-lg relative overflow-hidden">
            {!user?.premium && (
              <div className="absolute inset-0 bg-background/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-4 text-center">
                <Lock className="h-6 w-6 text-muted-foreground mb-2" /><p className="text-[10px] font-bold uppercase mb-2">{t('userScore.premiumLabel')}</p>
                <Button asChild size="sm" className="h-7 text-[10px] px-2"><Link href="/premium"><Gem className="h-3 w-3 mr-1" /> {t('userScore.premiumLabel')}</Link></Button>
              </div>
            )}
        ...
        {stats && workout !== "0" && (
          <Card className="bg-muted/30 border-dashed">
            <CardContent className="flex flex-wrap items-center justify-center gap-8 py-4">
              <div className="flex items-center gap-2"><InfoIcon className="h-4 w-4 text-primary" /><span className="text-sm font-semibold">{t('recap.maxReps')}:</span><Badge variant="secondary" className="bg-primary/10 text-primary">{stats.maxRepsInSample || "N/A"} reps</Badge></div>
              {stats.inferredTimeCap && <div className="flex items-center gap-2"><CalendarIcon className="h-4 w-4 text-primary" /><span className="text-sm font-semibold">Top 100 Pace:</span><Badge variant="secondary" className="bg-primary/10 text-primary">{stats.inferredTimeCap}</Badge></div>}
            </CardContent>
          </Card>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <Card className="shadow-lg overflow-hidden h-[450px]">
            ...
            <CardContent className="pt-6 h-full"><div className="h-[300px] w-full">{!isMounted || isLoading ? <div className="h-full w-full flex items-center justify-center bg-muted/10 rounded-md"><LoaderCircle className="h-8 w-8 animate-spin text-muted-foreground" /></div> : <ResponsiveContainer width="100%" height="100%" minHeight={300}><ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: -20 }}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis type="number" dataKey="x" name={scatterMetric} domain={scatterConfig[scatterMetric].domain as any} stroke="hsl(var(--muted-foreground))" fontSize={10} /><YAxis type="number" dataKey="y" name="Rank" reversed domain={['auto', 'auto']} stroke="hsl(var(--muted-foreground))" fontSize={10} /><Tooltip cursor={{ strokeDasharray: '3 3' }} contentStyle={{ backgroundColor: 'hsl(var(--card))', borderRadius: '8px', border: '1px solid hsl(var(--border))', color: 'hsl(var(--foreground))' }} /><Scatter name="Athletes" data={scatterData}>{scatterData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.y <= 10 ? '#eab308' : 'hsl(var(--primary))'} />)}</Scatter></ScatterChart></ResponsiveContainer>}</div><div className="flex justify-between px-10 text-[10px] text-muted-foreground uppercase font-bold tracking-widest mt-2"><span>{scatterConfig[scatterMetric].label}</span><span>{workout === "0" ? t('leaderboard.overallRank') : t('leaderboard.workoutRank')}</span></div></CardContent>
          </Card>
        </div>

        <Card className="shadow-lg overflow-hidden">
          <CardHeader className="bg-muted/30 border-b flex flex-row items-center justify-between"><div><CardTitle className="flex items-center gap-2 text-lg"><ListOrdered className="h-5 w-5 text-primary" /> {t('leaderboard.title')}</CardTitle><CardDescription>{workout === "0" ? t('leaderboard.descriptionOverall') : t('leaderboard.descriptionWorkout', { year, workout })}</CardDescription></div></CardHeader>
          <CardContent className="p-0">
            {!isMounted || isLoading ? <div className="p-8 flex justify-center"><LoaderCircle className="animate-spin h-8 w-8 text-muted-foreground" /></div> : <Table><TableHeader><TableRow><TableHead className="w-[50px]"></TableHead><TableHead className="w-[100px]">{t('leaderboard.rank')}</TableHead><TableHead>{t('leaderboard.name')}</TableHead><TableHead className="text-right">{workout === "0" ? t('leaderboard.points') : t('leaderboard.score')}</TableHead></TableRow></TableHeader><TableBody>{athletesList.map((athlete, idx) => (
        ...
        <div className="bg-muted/30 p-4 rounded-lg flex items-start gap-3 border border-border/50">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            <p className="mb-2"><strong>{t('methodology.unificationTitle')}</strong> : {t('methodology.unificationDesc')}</p>
            <p><strong>{t('methodology.overallTitle')}</strong> : {t('methodology.overallDesc')}</p>
          </div>
        </div>
      </main>
    </div>
